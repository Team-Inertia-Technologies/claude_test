...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case "get_file": {
        const { file_key, depth, version, geometry } = args as {
          file_key: string;
          depth?: number;
          version?: string;
          geometry?: string;
      ...
      case "get_file_nodes": {
        const { file_key, node_ids, depth, version, geometry } = args as {
          file_key: string;
          node_ids: string[];
          depth?: number;
          version?: string;
      ...
      case "get_file_components": {
        const { file_key } = args as { file_key: string };
      ...
      case "get_images": {
        const { file_key, node_ids, scale, format, svg_include_id, svg_simplify_stroke, use_absolute_bounds, version } = args as {
          file_key: string;
          node_ids: string[];
          scale?: number;
      ...
      case "post_comment": {
        const { file_key, message, x, y, node_id, node_offset_x, node_offset_y } = args as {
          file_key: string;
          message: string;
          x?: number;
          y?: number;
          ...
          node_offset_y?: number;
        };
        let client_meta: Parameters<typeof client.postComment>[2];
      ...
      case "delete_comment": {
        const { file_key, comment_id } = args as { file_key: string; comment_id: string };
      ...
      case "get_team_components": {
        const { team_id, page_size, after, before } = args as {
          team_id: string;
          page_size?: number;
          after?: number;
          before?: number;
